...
router.get('/dashboard', authenticate, async (req, res) => {
    try {
        ...
        // Get counts
        const [totalEmployees, activeEmployees, totalTasks, pendingTasks, inProgressTasks, completedTasks, todayAttendance, pendingLeaves, upcomingCertifications] = await Promise.all([
    ...
    try {
        ...
        res.json({
            leaves: leaves.map(l => ({
                ...
                endDate: l.endDate,
                duration: Math.ceil((new Date(l.endDate).getTime() - new Date(l.startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1
            })),
            statistics: {
